...
def test():
    ...
    bmc_periphery_address = sp.test_account("peripheri")
    ...
    bmcManagement_contract.enable().run(sender=creator)
    ...
    bmcManagement_contract.set_bmc_periphery(bmc_periphery_address.address).run(sender=creator)
    ...
    bmcManagement_contract.set_bmc_periphery(bmc_periphery_address.address).run(sender=creator, valid=False, exception="AlreadyExistsBMCPeriphery")
    # Verify that bmc_periphery is set to the valid address
    sc.verify(bmcManagement_contract.data.bmc_periphery.is_some() == True)
    sc.verify(bmcManagement_contract.data.bmc_periphery.open_some() == bmc_periphery_address.address)
    ...
    # should work
    bmcManagement_contract.add_link("btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnB").run(sender=creator)
    # add_link by of same link should fail
    bmcManagement_contract.add_link("btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnB").run(sender=creator, valid=False, exception="AlreadyExistsLink")
    ...
    bmcManagement_contract.remove_link("btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnW").run(sender=creator)


    # test case 10: get_links function
    link_to_compare = bmcManagement_contract.get_links()
    added_link = sp.list(['btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnA', 'btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnB'])
    ...
    bmcManagement_contract.set_link_rx_height(link="btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnW", height=height).run(sender=creator, valid=False, exception="NotExistsKey")
    ...
    next_link = "btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hLINK"
    ...
    sc.verify_equal(get_routes, sp.map({0: sp.record(dst=sp.string("btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hDEST"), next=sp.string("btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hLINK"))}))
    ...
    get_link = bmcManagement_contract.get_link(sp.string('btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnA'))
    data = sp.record(
    ...
    # resolve_route = bmcManagement_contract.resolve_route(sp.string('77.tezos'))
    # sc.verify_equal(resolve_route, sp.pair('btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hLINK', 'btp://77.tezos/tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hDEST'))




def deploy_bmcManagement_contract():
    bmcManagement_contract = BMCManagement.BMCManagement()
