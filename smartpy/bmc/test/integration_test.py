...
def test():
    ...
    bts_periphery = deploy_btsPeriphery_Contract(btsCore_contract.address, helper_contract.address, parse_address.address, bmcPeriphery_contract.address)
    sc += bts_periphery

    #set bmc periphery
    bmcManagement_contract.set_bmc_periphery(bmcPeriphery_contract.address).run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    #set bmc_btp_address(netwrk address)
    bmcManagement_contract.set_bmc_btp_address("NetXnHfVqm9iesp.tezos").run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    ...
    #add_service
    svc1 = sp.string("bts")
    bmcManagement_contract.add_service(sp.record(addr=bts_periphery.address, svc=svc1)).run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    ...
    bmcManagement_contract.add_route(sp.record(dst=dst, link=next_link)).run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))

    # bmcManagement_contract.add_route(sp.record(dst="btp://NetXnHfVqm9iesp.tezos/cxff8a87fde8971a1d10d93dfed3416b0a6258dest", link="btp://NetXnHfVqm9iesp.tezos/cxff8a87fde8971a1d10d93dfed3416b0a6258d67b")).run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))


    #add_link
    bmcManagement_contract.add_link("btp://0x7.icon/cxff8a87fde8971a1d10d93dfed3416b0a6258d67b").run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))


    # #remove_link
    # bmcManagement_contract.add_link("btp://0x7.icon/cxff8a87fde8971a1d10d93dfed3416b0a6258dead").run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    # bmcManagement_contract.remove_link("btp://0x7.icon/cxff8a87fde8971a1d10d93dfed3416b0a6258dead").run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    ...
    #add_relay
    bmcManagement_contract.add_relay(sp.record(link="btp://0x7.icon/cxff8a87fde8971a1d10d93dfed3416b0a6258d67b", addr=sp.set([sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9")]))).run(sender=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"))
    ...
    # btsCore_contract.transfer_native_coin("btp://0x7.icon/cx4419cb43f1c53db85c4647e4ef0707880309726d").run(sender= sp.address("tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnW"), amount=sp.tez(30), valid=False, exception="FailCheckTransfer")

    btsCore_contract.transfer_native_coin("btp://0x7.icon/cx4419cb43f1c53db85c4647e4ef0707880309726d").run(sender= sp.address("tz1e2HPzZWBsuExFSM4XDBtQiFnaUB5hiPnW"), amount=sp.tez(30))

    # handle_relay_message
    msg=sp.bytes("")
    ...
    # sc.verify_equal(bmcPeriphery_contract.get_status("btp://0x7.icon/cxff8a87fde8971a1d10d93dfed3416b0a6258d67b"), sp.record(current_height = 0, rx_height = 2, rx_seq = 0, tx_seq = 6))
...
def deploy_btsPeriphery_Contract(core_address, helper, parse, bmc):
    btsPeriphery_Contract = BTSPeriphery.BTPPreiphery(bmc_address= bmc, bts_core_address=core_address, helper_contract=helper, parse_address=parse, owner_address=sp.address("tz1XGbmLYhqcigxFuBCJrgyJejnwkySE4Sk9"), native_coin_name="btp-NetXnHfVqm9iesp.tezos-XTZ")
